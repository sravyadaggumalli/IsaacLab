...
# (observations, rewards, terminations, events) using custom
...
##
# Custom MDP Configurations for the Blue Cube Lifting & Placing Task
##
...
class BlueCubeLiftPlaceObservationsCfg:
    ...
    class PolicyCfg(ObsGroup):
        ...
        blue_cube_position = ObsTerm(
            ...
            params={"object_cfg": SceneEntityCfg("cube_blue")},
        )
        red_cube_position = ObsTerm(
            func=mdp.object_position_in_robot_root_frame,
            params={"object_cfg": SceneEntityCfg("cube_red")},
        )
        green_cube_position = ObsTerm(
            func=mdp.object_position_in_robot_root_frame,
            params={"object_cfg": SceneEntityCfg("cube_green")},
        )
        target_placement_position = ObsTerm(
            func=mdp.generated_commands, params={"command_name": "placement_pose"}
        )
...
class BlueCubeLiftPlaceEventCfg:
    ...
    reset_blue_cube_position = EventTerm(
        func=mdp.reset_root_state_uniform,
        mode="reset",
        params={
            "pose_range": {"x": (0.25, 0.35), "y": (-0.05, 0.05), "z": (0.02, 0.025)},
            "velocity_range": {},
            "asset_cfg": SceneEntityCfg("cube_blue"),
        },
    )
    reset_red_cube_position = EventTerm(
        ...
        mode="reset",
        params={
            "pose_range": {"x": (0.2, 0.3), "y": (-0.15, -0.05), "z": (0.02, 0.025)},
            "velocity_range": {},
            "asset_cfg": SceneEntityCfg("cube_red"),
        },
    )
    reset_green_cube_position = EventTerm(
        ...
        mode="reset",
        params={
            "pose_range": {"x": (0.2, 0.3), "y": (0.05, 0.15), "z": (0.02, 0.025)},
            "velocity_range": {},
            "asset_cfg": SceneEntityCfg("cube_green"),
        },
    )
...
class BlueCubeLiftPlaceRewardsCfg:
    ...
    action_rate = RewTerm(func=mdp.action_rate_l2, weight=-1e-6)  # prior value: -1e-4
    joint_vel = RewTerm(
    ...
    grasping_red_cube_penalty = RewTerm(
        func=mdp.penalize_wrong_grasp,
        params={
            "robot_cfg": SceneEntityCfg("robot"),
            "ee_frame_cfg": SceneEntityCfg("ee_frame"),
            "distractor_cube_cfg": SceneEntityCfg("cube_red"),
            "grasp_dist_thresh": 0.06,
            "closed_joint_pos_thresh": 0.01,
        },
        weight=-5.0,
    )
    grasping_green_cube_penalty = RewTerm(
        func=mdp.penalize_wrong_grasp,
        params={
            "robot_cfg": SceneEntityCfg("robot"),
            "ee_frame_cfg": SceneEntityCfg("ee_frame"),
            "distractor_cube_cfg": SceneEntityCfg("cube_green"),
            "grasp_dist_thresh": 0.06,
            "closed_joint_pos_thresh": 0.01,
        },
        weight=-5.0,
...
class BlueCubeLiftPlaceTerminationsCfg:
    ...
    blue_cube_dropping = DoneTerm(
        ...
        params={"minimum_height": -0.05, "asset_cfg": SceneEntityCfg("cube_blue")},
    )
...
class FrankaBlueCubeLiftEnvCfg(LiftEnvCfg):
    ...
    def __post_init__(self):
        ...
        init_z = 0.0203
        self.scene.cube_blue = RigidObjectCfg(
            prim_path="{ENV_REGEX_NS}/Cube_Blue",
            init_state=RigidObjectCfg.InitialStateCfg(
                pos=[0.5, 0.0, init_z], rot=[1, 0, 0, 0]
            ),
            spawn=UsdFileCfg(
                usd_path=f"{ISAAC_NUCLEUS_DIR}/Props/Blocks/blue_block.usd",
                scale=(1.0, 1.0, 1.0),
                rigid_props=cube_properties,
                semantic_tags=[("class", "target_cube"), ("class", "cube_blue")],
            ),
        )
        self.scene.cube_red = RigidObjectCfg(
            prim_path="{ENV_REGEX_NS}/Cube_Red",
            init_state=RigidObjectCfg.InitialStateCfg(
                pos=[0.4, -0.1, init_z], rot=[1, 0, 0, 0]
            ),
            spawn=UsdFileCfg(
                usd_path=f"{ISAAC_NUCLEUS_DIR}/Props/Blocks/red_block.usd",
                scale=(1.0, 1.0, 1.0),
                rigid_props=cube_properties,
                semantic_tags=[("class", "distractor_cube"), ("class", "cube_red")],
            ),
        )
        self.scene.cube_green = RigidObjectCfg(
            ...
            spawn=UsdFileCfg(
                ...
                scale=(1.0, 1.0, 1.0),
                rigid_props=cube_properties,
                semantic_tags=[("class", "distractor_cube"), ("class", "cube_green")],
            ),
        )
